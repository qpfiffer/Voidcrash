...
extern unit		betray_plugin_audio_unit_create(); /* you need to create a autio unit id in order to create callbacks*/
...
extern void		betray_plugin_button_set(uint id, boolean press, uint character); /* Call this Function to set the current state of a button. */
extern void		betray_plugin_button_free(uint id); /* Frees the button id, if the device is nolonger active. */

extern uint		betray_plugin_pointer_allocate(uint user_id, uint code, char *name); /* Allocate a pointer id for anew pointer. */ 
extern uint		betray_plugin_pointer_set(uint code, float x, float y, boolean *buttons); /* Set the current state of the button.,*/
extern void		betray_plugin_pointer_free(uint id); /* Frees the pointer id, if the device is nolonger active. */

extern uint		betray_plugin_axis_allocate(uint user_id, char *name, BAxisType type, uint axis_count);  /* Allocates a new axis, and describes its user id, name type and axis count. */
