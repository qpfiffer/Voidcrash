...
extern void			(*betray_plugin_callback_set_audio_read)(uint audio_unit_id, uint (*func)(void *data, uint type, uint channel, uint buffer_size), uint channels, float *vectors);
...
extern uint			(*betray_plugin_pointer_allocate)(uint user_id, uint button_count, char *name);
