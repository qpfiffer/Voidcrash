...
void		(*betray_plugin_callback_set_audio_read)(uint audio_unit_id, uint (*func)(void *data, uint type, uint channel, uint buffer_size), uint channels, float *vectors) = NULL;
...
uint		(*betray_plugin_pointer_allocate)(uint user_id, uint button_count, char *name) = NULL;
uint		(*betray_plugin_pointer_set)(uint code, float x, float y, boolean *buttons) = NULL;
void		(*betray_plugin_pointer_free)(uint id) = NULL;
uint		(*betray_plugin_axis_allocate)(uint user_id, char *name, BAxisType type, uint axis_count) = NULL;
