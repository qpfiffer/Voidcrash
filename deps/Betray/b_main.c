...
typedef struct{
	...
	uint (*read)(void *data, uint type, uint channel, uint buffer_size);
	uint channels;
	float *vectors;
}BAudioCallback;
...
void betray_plugin_callback_set_audio_read(uint audio_unit_id, uint (*func)(void *data, uint type, uint channel, uint buffer_size), uint channels, float *vectors)
{
	uint i;
	if(audio_unit_id < BETRAY_AUDIO_UNIT_MAGIC_NUMBER || audio_unit_id >= BETRAY_AUDIO_UNIT_MAGIC_NUMBER + BGlobal.plugins.audio_count)
...
uint betray_audio_read(uint unit_id, void *data, uint type, uint channels, uint buffer_size)
{
	int8 *data8 = NULL;
	int16 *data16 = NULL;
	int32 *data32 = NULL;
	...
	if(count == unit_id)
	{
		switch(type)
		{
			...
			data8 = (int8 *)data;
			return BGlobal.plugins.audio[i].read(&data8[channels], type, channels, buffer_size);
			case BETRAY_TYPE_INT16 :
			data16 = (int16 *)data;
			return BGlobal.plugins.audio[i].read(&data16[channels], type, channels, buffer_size);
			case BETRAY_TYPE_INT32 :
			data32 = (int32 *)data;
			return BGlobal.plugins.audio[i].read(&data32[channels], type, channels, buffer_size);
			case BETRAY_TYPE_FLOAT32 :
			dataf32 = (float *)data;
			return BGlobal.plugins.audio[i].read(&dataf32[channels], type, channels, buffer_size);
		}
	}
	return 0;
}
...
//	betray_context_type = context_type;
	...
	#ifdef BETRAY_WIN32_SYSTEM_WRAPPER
	if(window_fullscreen && (window_size_x != display_mode_x || window_size_y != display_mode_y))
		b_win32_screen_mode(window_size_x, window_size_y);
	BGlobal.sterioscopic = BGlobal_occular_distance < 0.00001 || BGlobal_occular_distance > -0.00001;
...
/*{
	...
	for(i = 0; imagine_path_search(".dll", TRUE, IMAGINE_DIR_HOME_PATH, FALSE, i, path, 64); i++)
	{
