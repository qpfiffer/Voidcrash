...
extern uint	betray_audio_util_stream_buffer_left(uint stream, uint sound);
...
uint betray_audio_read_func(void *data, uint type, uint channel, uint buffer_size)
{
	int8 *pint8;
	int16 *pint16, *read;
	int32 *pint32;
	...
	for(i = 0; i < buffer_size && b_audio_win_blocks_collected_in != 0;)
	{
		...
		switch(type)
		{
			case BETRAY_TYPE_INT8 :
				...
				{
					pint8[i * channel] = (uint8)(read[b_audio_win_collected_in_progress] / 256);
					b_audio_win_collected_in_progress++;
					pint8[i * channel + 1] = (uint8)(read[b_audio_win_collected_in_progress + 1] / 256);
					b_audio_win_collected_in_progress++;
				}
			break;
			case BETRAY_TYPE_INT16 :
				...
				for(; i < buffer_size && b_audio_win_collected_in_progress < BETRAY_AUDIO_BLOCK_BYTE_SIZE / 2; i++)
				{
					pint16[i * channel] = read[b_audio_win_collected_in_progress];
					b_audio_win_collected_in_progress++;
					pint16[i * channel + 1] = read[b_audio_win_collected_in_progress + 1];
					b_audio_win_collected_in_progress++;
				}
			break;
			case BETRAY_TYPE_INT32 :
				...
				for(; i < buffer_size && b_audio_win_collected_in_progress < BETRAY_AUDIO_BLOCK_BYTE_SIZE / 2; i++)
				{
					pint32[i * channel] = (int32)read[b_audio_win_collected_in_progress] * 256 * 256;
					b_audio_win_collected_in_progress++;
					pint32[i * channel + 1] = (int32)read[b_audio_win_collected_in_progress + 1] * 256 * 256;
					b_audio_win_collected_in_progress++;
				}
			break;
			case BETRAY_TYPE_FLOAT32 :
				...
				for(; i < buffer_size && b_audio_win_collected_in_progress < BETRAY_AUDIO_BLOCK_BYTE_SIZE / 2; i++)
				{
					preal32[i * channel] = (float)read[b_audio_win_collected_in_progress] / (128 * 256 - 1);
					b_audio_win_collected_in_progress++;
					preal32[i * channel + 1] = (float)read[b_audio_win_collected_in_progress + 1] / (128 * 256 - 1);
					b_audio_win_collected_in_progress++;
				}
			break;
		}
