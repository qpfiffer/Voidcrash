...
void controller_plugin_callback_main(BInputState *input)
{
	...
	for(j = 0; j < 4; j++)
	{
		if(ERROR_SUCCESS == XInputGetState(j, &pState))
		{
			...
			{
				controller_code_id[j][0] = betray_plugin_axis_allocate(controller_user_ids[j], "Left Stick", B_AXIS_STICK, 2);
				controller_code_id[j][1] = betray_plugin_axis_allocate(controller_user_ids[j], "Right Stick", B_AXIS_STICK, 2);
				controller_code_id[j][2] = betray_plugin_axis_allocate(controller_user_ids[j], "Left Bumper", B_AXIS_BUMPER_LEFT, 1);
				controller_code_id[j][3] = betray_plugin_axis_allocate(controller_user_ids[j], "Right Bunper", B_AXIS_BUMPER_RIGHT, 1);
