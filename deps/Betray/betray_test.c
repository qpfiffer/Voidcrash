...
//		if(sound_source == -1)
//			sound_source = betray_audio_sound_play(b_test_sound, pos, vec, 1, 0.8, TRUE, FALSE, boolean auto_delete); /* play a sound (ambient equals fixed position to head)*/
	...
	if(input->mode == BAM_DRAW)
	{
		...
		for(i = 0; i < 11; i++)
		{
			for(j = 0; j < 11; j++)
			{
				...
				{
					glPushMatrix();
					glTranslatef(((float)i - 5.0), ((float)j - 5.0), ((float)k - 5.0));
		...
		for(i = 0; i < input->axis_count; i++)
		{
			switch(input->axis[i].axis_count)
			{
				case 1 :
					...
					b_test_draw_line(f + 0.01, -0.3 - 0.11,
									f - 0.01, -0.3 - 0.11, 0.4, 0.4, 0.4);
					b_test_draw_line(f, -0.3, f, input->axis[i].axis[1] * 0.1 - 0.3, 1, 1, 1);
				break;
				case 2 :
				case 3 :
					glPushMatrix();
