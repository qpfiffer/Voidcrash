...
void f_debug_mem_add(void *pointer, uint size, char *file, uint line)
{
	...
	}else
	{
		if(i < 1024)
		{
			...
			f_allock_lines[i].alloc_alocated = 256;
...
void *f_debug_mem_realloc(void *pointer, uint size, char *file, uint line)
{
	...
	{
		for(j = 0; j < f_allock_lines[i].alloc_count; j++)
			if(f_allock_lines[i].allocs[j].buf == pointer)
				break;
		if(j < f_allock_lines[i].alloc_count)
			break;
	}
	if(i == f_allock_line_count)
	{
	...
	move = f_allock_lines[i].allocs[j].size;
	
	if(move > size)
		move = size;

	pointer2 = f_debug_mem_malloc(size, file, line);
	memcpy(pointer2, pointer, move);
	f_debug_mem_free(pointer);
	return pointer2;
}
