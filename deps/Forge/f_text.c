...
boolean	f_text_filter_nocaps(char *text, char *filter)
{
	uint i, j;
	for(i = 0; text[i] != 0; i++)
	{
		if(text[i] == filter[0] || filter[0] + 32 == text[i] || filter[0] - 32 == text[i])
		{
			for(j = 0; text[i + j] != 0 && filter[j] != 0 && (text[i + j] == filter[j] || (text[i + j] > 64 && text[i + j] < 91 && text[i + j] + 32 == filter[j]) || (filter[j] > 64 && filter[j] < 91 && filter[j] + 32 == text[i + j])); j++);
			if(filter[j] == 0)
				return TRUE;
		}
	}
	for(i = 0; text[i] != 0; i++)
	{
		if(text[i] == filter[0])
		{
			for(j = 0; text[i + j] != 0 && filter[j] != 0 && (text[i + j] == filter[j]); j++);
			if(filter[j] == 0)
				return TRUE;
		}
	}
