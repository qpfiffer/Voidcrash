...
void r_primitive_init()
{
	...
	r_line_pool = r_array_allocate(SUI_DRAW_LINE_BUFFER, vertex_format_types, vertex_format_size, 2);
	r_surface_pool = r_array_allocate(6, vertex_format_types, vertex_format_size, 1);
	r_image_pool = r_array_allocate(SUI_DRAW_IMAGE_BUFFER, vertex_format_types, vertex_format_size_surface, 1);
	r_sprite_pool = r_array_allocate(SUI_DRAW_SPRITE_BUFFER, vertex_format_types, vertex_format_size, 3);
	...
	r_color_shader = r_shader_create(NULL, 0, r_shader_color_vertex, r_shader_color_fragment, "color primitive");
	r_color_location_color = r_shader_uniform_location(r_color_shader, "color");

	r_vertex_color_shader = r_shader_create(NULL, 0, r_shader_vertex_color_vertex, r_shader_vertex_color_fragment, "vertex color primitive");

	r_surface_shader = r_shader_create(NULL, 0, r_shader_surface_vertex, r_shader_surface_fragment, "color primitive");
	...
	r_image_shader = r_shader_create(NULL, 0, r_shader_image_vertex, r_shader_image_fragment, "image primitive");
	...
	r_texture_shader = r_shader_create(NULL, 0, r_shader_texture_vertex, r_shader_texture_fragment, "texture");

	r_sprite_shader = r_shader_create(NULL, 0, r_shader_sprite_vertex, r_shader_sprite_fragment, "sprite primitive");
}
...
void r_primitive_image(float pos_x, float pos_y, float pos_z, float size_x, float size_y, float u_start, float v_start, float u_end, float v_end, uint texture_id, float red, float green, float blue, float alpha)
{
	...
	r_shader_vec4_set(r_image_location_uv, u_start, v_start, u_end - u_start, v_end - v_start);
	r_shader_vec4_set(r_image_location_color, red, green, blue, alpha);
	m = r_matrix_get();
	r_matrix_push(m);
	r_matrix_translate(m, pos_x, pos_y, pos_z);
	r_matrix_scale(m, size_x, size_y, 0);
	r_shader_texture_set(r_image_shader, 0, texture_id);
...
void r_primitive_surface2(float pos_x, float pos_y, float pos_z, float size_x, float size_y, float red, float green, float blue, float alpha)
{
	...
	r_shader_vec4_set(r_color_location_color, red, green, blue, alpha);
	m = r_matrix_get();
	r_matrix_push(m);
	r_matrix_translate(m, pos_x, pos_y, pos_z);
	r_matrix_scale(m, size_x, size_y, 0);
...
void r_primitive_surface(float pos_x, float pos_y, float pos_z, float size_x, float size_y, float red, float green, float blue, float alpha)
{
	...
	RMatrix *m;
	r_shader_set(r_color_shader);
	r_shader_vec4_set(r_color_location_color, 1, 1, 1, 1);
	m = r_matrix_get();
	r_matrix_push(m);
...
r_shader_vec4_set(r_color_location_color, 0.5, 0.5, 0.5, 0.1);
	r_array_section_draw(r_surface_pool, NULL, GL_TRIANGLES, 0, 6);
