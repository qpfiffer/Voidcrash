...
void (APIENTRY *p_glDrawBuffersARB)(GLuint, GLenum *buffers) = NULL;

void (APIENTRY *p_glTexImage3D)(GLenum target, int level, GLenum internalformat, uint width, uint height, uint depth, int border, GLenum format, GLenum type, const void* pixels) = NULL;
...
void r_framebuffer_status(void)
{
    GLenum status;

    if (!p_glBindFramebufferEXT)
        return;
    status = p_glCheckFramebufferStatusEXT(GL_FRAMEBUFFER_EXT);
    switch(status)
	{
        case GL_FRAMEBUFFER_COMPLETE_EXT:
			printf("FRAME BUFFER WORKING!\n");
			return;
            break;
...
uint r_framebuffer_allocate(boolean multilayer, uint *buffers, uint buffer_count, uint depth_buffer)
{
	...
	for(i = 0; i < buffer_count; i++)
		p_glFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT, GL_COLOR_ATTACHMENT0_EXT + i, GL_TEXTURE_2D, buffers[i], 0);
	if(depth_buffer != -1)
		p_glFramebufferRenderbufferEXT(GL_FRAMEBUFFER_EXT, GL_DEPTH_ATTACHMENT_EXT, GL_RENDERBUFFER_EXT, depth_buffer);
	r_framebuffer_status();
...
*/	p_glBindFramebufferEXT(GL_FRAMEBUFFER_EXT, id);	
}

void r_framebuffer_free(uint fbo)
{
	p_glDeleteFramebuffersEXT(1, &fbo);
}
...
boolean r_framebuffer_init(void)
{
	...
	{
		p_glTexImage3D = r_extension_get_address("glTexImage3D");
	}
	if(r_extension_test("GL_ARB_draw_buffers"))
		p_glDrawBuffersARB = r_extension_get_address("glDrawBuffersARB");

	if(r_extension_test("GL_EXT_framebuffer_object"))
	{
		p_glBindFramebufferEXT = r_extension_get_address("glBindFramebufferEXT");
		p_glDeleteFramebuffersEXT = r_extension_get_address("glDeleteFramebuffersEXT");
		p_glGenFramebuffersEXT = r_extension_get_address("glGenFramebuffersEXT");
		p_glBindRenderbufferEXT = r_extension_get_address("glBindRenderbufferEXT");
		p_glDeleteRenderbuffersEXT = r_extension_get_address("glDeleteRenderbuffersEXT");
		p_glGenRenderbuffersEXT = r_extension_get_address("glGenRenderbuffersEXT");
		p_glCheckFramebufferStatusEXT = r_extension_get_address("glCheckFramebufferStatusEXT");
		p_glRenderbufferStorageEXT = r_extension_get_address("glRenderbufferStorageEXT");
		p_glFramebufferTexture2DEXT = r_extension_get_address("glFramebufferTexture2DEXT");
		p_glFramebufferRenderbufferEXT = r_extension_get_address("glFramebufferRenderbufferEXT");
		return TRUE;
	}
...
uint r_texture_allocate(PImageFormat format, uint x, uint y, uint z, boolean filter, boolean tile, void *data)
{
	...
	if(p_glTexImage3D == NULL && z != 1 && z != R_IB_IMAGE_CUBE)
	{
	...
	{
		p_glGenRenderbuffersEXT(1, &texture_id);		
		p_glBindRenderbufferEXT(GL_RENDERBUFFER_EXT, texture_id);
		if(format == R_IF_DEPTH16)
			p_glRenderbufferStorageEXT(GL_RENDERBUFFER_EXT, GL_DEPTH_COMPONENT16, x, y);
		else if(format == R_IF_DEPTH24)
			p_glRenderbufferStorageEXT(GL_RENDERBUFFER_EXT, GL_DEPTH_COMPONENT24, x, y);
		else if(format == R_IF_DEPTH32)
			p_glRenderbufferStorageEXT(GL_RENDERBUFFER_EXT, GL_DEPTH_COMPONENT32, x, y);
		else
			p_glRenderbufferStorageEXT(GL_RENDERBUFFER_EXT, GL_DEPTH24_STENCIL8_EXT, x, y);
		return texture_id;
	}else
	{
		...
		if(dimensions == GL_TEXTURE_CUBE_MAP_EXT)
		{
			...
			p_glTexImage3D(dimensions, 0, internal_format, x, y, z, 0, input_format, data_type, data);
		else
