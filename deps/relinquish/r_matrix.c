...
{
	float center[4], x_axis[4], y_axis[4], z_axis[4], out[4];
	float *model;
	if(matrix == NULL)
		matrix = r_matrix_state;
	out[0] = x * z;
...
{
	double center[3], x_axis[3], y_axis[3], z_axis[3], out[3], m[16];
	float *model;
	uint i;
	if(matrix == NULL)
		matrix = r_matrix_state;
...
{
	float camera[3], pointer[3], axis_vec[3] = {0, 0, 0};
	if(matrix == NULL)
		matrix = r_matrix_state;
...
{
	float f, camera[3], pointer[3], axis_vec[3], normalized[3];
	if(matrix == NULL)
		matrix = r_matrix_state;
	r_matrix_projection_worldf(matrix, pointer, x, y, -1);
	r_matrix_projection_worldf(matrix, camera, 0, 0, 0);
